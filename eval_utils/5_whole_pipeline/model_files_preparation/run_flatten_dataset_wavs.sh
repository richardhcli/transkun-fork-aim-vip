...
#SBATCH --job-name flatten_wavs
#SBATCH --output=/scratch/gilbreth/li5042/transkun/transkun_fork/eval_utils/5_whole_pipeline/output/flatten_wavs_dataset.out
#SBATCH --error=/scratch/gilbreth/li5042/transkun/transkun_fork/eval_utils/5_whole_pipeline/output/flatten_wavs_dataset.err
...
echo "Starting one-time dataset WAV flattening..."

echo "Granting read and write permissions to datasets directory..."
chmod -R u+rw "$DATASETS_DIR"

# Flatten script
python "$SCRIPT_DIR/flatten_dataset_wavs.py" \
  --dataset-dir "$DATASETS_DIR" \
  --output-json "$LOG_DIR/flatten_wav_log.json" \
  --output-log "$LOG_DIR/flatten_wav_script.log"
...
log_file = Path("/scratch/gilbreth/li5042/transkun/transkun_fork/eval_utils/5_whole_pipeline/output/flatten_logs/flatten_wav_log.json")
...
print(f"Skipped (already mono): {skipped}")
...
EOF
    ...
    echo "Error detected during Python script execution. Check flatten_wav_script.log."
fi
