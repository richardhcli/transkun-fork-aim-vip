...
from single_file_transcription import single_file_transcription
...
def transcribe_file(wav_file, pred_file, gpu_queue, state_manager, error_log, checkpoint_pt, model_conf):
    ...
    if checkpoint_pt is not None:
        kwargs["checkpoint_pt"] = checkpoint_pt
    if model_conf is not None:
        kwargs["model_conf"] = model_conf

    transcription_command_result = single_file_transcription(str(wav_file), str(pred_file), device, **kwargs)

    if transcription_command_result != 0:
        status = "FAILED"
        # Log the exact error for debugging
        with open(error_log, "a") as f:
            f.write(f"[transcribe_all_multiGPU] CRASH: {wav_file.stem} on {device}\n")
            f.write(str(transcription_command_result))
            f.write("\n\n")
...
def main():
    ...
    parser.add_argument("--checkpoint_pt", required=False, default=None, help="Path to checkpoint.pt")
    ...
    print(f"[INFO] Parsing metadata from {csv_path} for 'test' split files...")
    ...
    with open(csv_path, mode='r', encoding='utf-8') as file:
        ...
        for row in reader:
            if row['split'] == 'test':
                # Resolve the relative path from the CSV to an absolute Path object
                full_wav_path = maestro_path / row['audio_filename']
                test_wav_files.append(full_wav_path)

    # Delegate task filtering to the State Manager
    tasks = []
    eligible_wavs = state_manager.get_eligible_tasks(test_wav_files)
    ...
    skip_count = len(test_wav_files) - len(tasks)
    total_files_to_transcribe = len(tasks)
    print(f"[INFO] State Check: Transcribing {total_files_to_transcribe} files. Found {len(test_wav_files)} test dataset files. Skipping {skip_count} files (already SUCCESS or max FAILED). ")
    ...
    checkpoint_pt = args.checkpoint_pt
    model_conf = None
    if checkpoint_pt:
        model_conf = str(Path(checkpoint_pt).parent / "model.conf")

    with ThreadPoolExecutor(max_workers=num_gpus) as executor:
        futures = {
            executor.submit(transcribe_file, wav, pred, gpu_queue, state_manager, error_log, checkpoint_pt, model_conf): wav 
